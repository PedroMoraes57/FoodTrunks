...
from .models import Venda, Produto, ItemVenda, Cliente, LocalAtendimento
...
def grafico_produtos_mais_vendidos(request):
    ...
    return render(
        request,
        "core/grafico_produtos_mais_vendidos.html",
        { 
            "labels": labels,
            "data": data,
        },
...
@login_required 
def portfolio_produtos(request): 
    produtos = Produto.objects.filter(ativo=True) 
    return render(request, 'core/portfolio_produtos.html', {'produtos': produtos}) 
